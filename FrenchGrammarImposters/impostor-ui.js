...
function updateTaskProgressBar() {
    ...
    // Hide task progress bar during defeat screens
    if (impostorGameState.gamePhase === 'game_over' || impostorGameState.gamePhase === 'tasks_completed') {
...
function updateCharactersDisplay() {
    ...
    // Hide character cards during defeat screens
    if (impostorGameState.gamePhase === 'game_over' || impostorGameState.gamePhase === 'tasks_completed') {
...
function updateGamePhaseDisplay() {
    ...
    switch (impostorGameState.gamePhase) {
        ...
        case 'impostor_victory':
            ...
            if (voteButtons) {
                // Get Hugo's color for the animated character
                ...
                voteButtons.innerHTML = `
                    <div class="victory-message">
                        ...
                        <!-- Laughing impostor animation -->
                        <div class="impostor-laugh-container">
        ...
        case 'game_over':
            if (voteTitle) voteTitle.innerHTML = '💀 Défaite !';
            if (voteInstructions) {
                voteInstructions.innerHTML = 'Vous avez été éjecté ! Améliorez votre français pour la prochaine fois.';
            }
            if (voteButtons) {
        ...
        case 'tasks_completed':
            if (voteTitle) voteTitle.innerHTML = '💀 Défaite !';
            if (voteInstructions) {
                voteInstructions.innerHTML = 'Les tâches ont été terminées ! Soyez plus rapide la prochaine fois.';
            }
            if (voteButtons) {
                // Get all impostors with their colors
                const allImpostors = impostorGameState.characters.filter(c => c.isImpostor);
